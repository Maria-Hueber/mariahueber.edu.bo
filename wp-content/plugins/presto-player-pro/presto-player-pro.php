...
// add pro components
add_filter('presto_player_pro_components', function ($components) use ($required_core_version) {
    ...
    if (!defined('PRESTO_TESTSUITE')) {
        ...
        if (version_compare(\PrestoPlayer\Plugin::version(), $required_core_version, '<')) {
            add_action('admin_notices', function () use ($required_core_version) {
    ...
    $pro_components = require_once 'inc/config/app.php';
