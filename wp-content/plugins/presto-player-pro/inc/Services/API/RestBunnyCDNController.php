...
use PrestoPlayer\Pro\Services\Bunny\StorageRequest;
...
class RestBunnyCDNController
{
    ...
    public function register_routes()
    {
         ...
         * Save API Key
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/api-key', [
            [
         ...
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/storage-zones', [
            [
                'methods'             => \WP_REST_Server::CREATABLE,
         ...
         * Create pull zones
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/pull-zones', [
            [
                'methods'             => \WP_REST_Server::CREATABLE,
         ...
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/upload', [
            'methods'             => \WP_REST_Server::CREATABLE,
         ...
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/public-videos', [
            [
         ...
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/private-videos', [
            [
                'methods'             => \WP_REST_Server::READABLE,
         ...
         * Sign urls
         */
        register_rest_route("{$this->namespace}/{$this->version}", '/' . $this->base . '/sign', [
            [
                'methods'             => \WP_REST_Server::CREATABLE,
    ...
    public function create_storage_zones(\WP_REST_Request $request)
    {
        ...
        if (!$created['private']) {
            $new = $this->cdn_api->post('storagezone', [
                'body' => [
    ...
    public function create_pull_zones(\WP_REST_Request $request)
    {
        ...
        // maybe create public
        if (!$created['public']) {
            $new = $this->cdn_api->post('pullzone', [
                'body' => [
    ...
    public function fetch_videos($type, \WP_REST_Request $request)
    {
        ...
        $bunnyCDNStorage = new StorageAPI($this->storage_zone["{$type}_name"], $password, 'ny');
        try {
        ...
        foreach ($items as $item) {
            ...
            $mimes = apply_filters('presto_player_allowed_mime_types', [
