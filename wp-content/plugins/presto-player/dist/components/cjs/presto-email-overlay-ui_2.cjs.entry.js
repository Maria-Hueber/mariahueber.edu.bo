...
const EmailOverlayUI = class {
  ...
  render() {
    return (index.h("div", { class: `overlay ${this.direction === 'rtl' ? 'rtl' : ''}`, ref: el => (this.textInput = el) }, index.h("div", { class: "content" }, index.h("h1", null, this.headline || this.defaultHeadline), this.isLoading ? (index.h("presto-player-spinner", null)) : (index.h("form", { onSubmit: e => this.handleSubmit(e) }, index.h("input", { type: "email", placeholder: this.placeholder, value: this.email, onInput: event => this.handleChange(event), required: true }), index.h("button", { type: "submit" }, this.buttonText || 'Play'))), this.errorMessage && index.h("p", { class: "error" }, this.errorMessage), this.bottomText && index.h("p", { innerHTML: this.bottomText }), !!this.allowSkip && (index.h("div", { class: "skip", onClick: () => {
        this.skip.emit();
      } }, this.skipText || 'Skip', " \u2192")))));
  }
};
